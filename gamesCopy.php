...
.repCard:hover{box-shadow:0 0 3px grey;}
...
.coinReward{font-size: 3em;
    font-family: fantasy;}
.dialog{background-color:white;color:lightgrey;
    padding:30px;
...
.dialog_item{color:black !important;}
</style>
<script>
...
$("#game_container").append('<div class="next" title="Passer à la carte suivante" onclick="suivant('+card_id+');">Suivant</div>');
$("#game_container").append('<div class="skip" title="Montrer la réponse" onclick="showCorrection('+game+','+card_id+');">Je donne ma langue au chat</div>');
...
$("#card_"+card_id+">.flipper>.back>.icons_card_container").append('<a target="_blank" class="icon_back google" href="http://www.google.com?q='+mot+'"></a>');
if(hasImage==1){$("#card_"+card_id+" > .flipper > .front").css("background-image","url(card_img/card_"+card_id+".png)");}
...
switch(game) {
  ...
  break;
  case "QCMimage2mot":
  $(".consigne").html("Selectionnez la carte qui correspont à l'image au centre.");
  console.log(game);
  //Mise en place de la grille
  $("#question").html("<table id='grilleContainer' style='display:inline-block;'><tr><td id='case_0'></td><td id='case_1'></td><td id='case_2'></td></tr><tr><td id='case_3'></td><td id='case_centre'></td><td id='case_4'></td></tr><tr><td id='case_5'></td><td id='case_6'></td><td id='case_7'></td></tr></table>")
  ...
  for(i in pile_glissante){
    repCard_id=pile_glissante[i];
    ...
    $("#repCard_"+repCard_id+">.flipper>.back>.icons_card_container").append('<a target="_blank" class="icon_back google" href="http://www.google.com?q='+repMot+'"></a>');
    if(repHasImage==1){$("#repCard_"+repCard_id+" > .flipper > .front").css("background-image","url(card_img/card_"+repCard_id+".png)");}
  ...
  $(".repCard").on("click",function(){
    ...
    if($(this).attr('id')=='repCard_'+card_id){$(this).find('.front').css("box-shadow","0 0 6px lime");bonne_rep(game,card_id);$(this).on("mouseleave",function(){suivant(card_id);});}
    else{$(this).find('.front').css("box-shadow","0 0 6px red");play_audio_fail();}
  })
  $('#correction').append('<div id="duree"></div>');
  break;
  case "QCMmot2image":
  $(".consigne").html("Selectionnez la carte qui correspont au mot écrit au centre.");
  console.log(game);
  //Mise en place de la grille
  $("#question").html("<table id='grilleContainer' style='display:inline-block;'><tr><td id='case_0'></td><td id='case_1'></td><td id='case_2'></td></tr><tr><td id='case_3'></td><td id='case_centre'></td><td id='case_4'></td></tr><tr><td id='case_5'></td><td id='case_6'></td><td id='case_7'></td></tr></table>")
  //mise en place de la carte au centre
  ...
  $(".repCard").on("click",function(){
    ...
    if($(this).attr('id')=='repCard_'+card_id){$(this).find('.back').css("background-color","lime");bonne_rep(game,card_id);$(this).on("mouseleave",function(){suivant(card_id);});}
    else{$(this).find('.back').css("background-color","red");play_audio_fail();}
  })
  $('#correction').append('<div id="duree"></div>');
  ...
  case "dictée":
    //affichage
    $("body").off();
    $(".consigne").html("Ecrire le mot après l'avoir entendu.");
    $("#reponse").html(questionCloze);//<input type="text" id="input_reponse" value="">');
    $("#reponse").append('<br><div class="boutton_envoi">Envoyer</div>');
    ...
    if(hasImage==1){
    $("#card_"+card_id+" > .flipper > .back").css("background-image","url(card_img/card_"+card_id+".png)");
    }
    else {
  ...
  case "bazarLettre":
    $("body").off();
    $(".consigne").html("Remettre les lettre de la phrase dans l'ordre.");
    ...
    //premiere lettre
    lettre_html='<div class="lettre" id='+0+' style="font-size: 1.8em;">'+repCloze[0]+'</div>';
    $("#lettre_container").append(lettre_html);
    //les autres lettres
    for (var i = 1; i < repCloze.length; i++) {
    lettre_html='<div class="lettre" id='+i+' style="font-size: 2em;">'+repCloze[i]+'</div>';
    ...
    $(".lettre").on("click", function() {
      ...
      if(repCloze[id_lettre].toLowerCase()==repCloze[rg_lettre_a_trouver].toLowerCase())//SI LA LETTRE TAPE EST BONNE
  ...
  //KEYPRESS on INPUT_REPONSE
    $("#input_reponse").off();
    $("#input_reponse").on("keyup",function(e){//si on a tapé une lettre au clavier
  ...
  case "bazarMot":
    ...
    //1er mot
    mot_html='<div class="mot_item" id='+0+' style="font-size: 1.8em;">'+list_mots[0]+'</div>';
...
$('#correction').append('<div id="duree"></div>');
  ...
  case "prononciation":
    $("body").off();
    $(".consigne").html("Cliquez sur le microphone et lisez le mot.");
    //affichage
  ...
  case "validation":
    $("body").off();
    $(".consigne").html("Ecrire le mot correspondant à la carte");
    $("#reponse").html(questionCloze);//<input type="text" id="input_reponse" value="">');
    $("#reponse").append('<br><div class="boutton_envoi">Envoyer</div>');
...
{
  $.getJSON("ajax.php?action=addActiviteGlobal&card_id="+card_id+"&game="+game, function(result){});

  if(game!="validation"){coinsReward=1;}else{coinsReward=3;}
  addCoin(coinsReward);
...
//s'il reste des id dans la grande pile, on en prend une au hasard que l'on met dans la pile glissante
  ...
  //if(game=="validation"){
    $.getJSON("ajax.php?action=cardLearned&card_id="+card_id+"&puissance="+puissance, function(result){
      console.log(result);
      //gestion durée
...
$('body').append("<div class='fenetreSombre' onclick='$(this).remove();'><div class='fenetreClaire' style='background-image:url(img/backgroundSky.png);background-size:cover;' onclick='event.stopPropagation();'></div></div>");
$('.fenetreClaire').append("<div id='rewardPage'><h3 class='ribbon'>Cartes complétées</h3></div>");
coinsReward=0;
var flagVal=0;
for(i in bilanRep)
  {
...
function bonne_lettre(card_id,id_lettre,rg_lettre)
{
  $('#input_reponse').val('');
    ...
    if(i<rg_lettre+1){$('#input_reponse').val($('#input_reponse').val()+mot[i]);}
    }
...
function checkAndReveal(transcript)
{
  ...
  speechResultArray=transcript.split(" ");
    console.log(transcript,speechResultArray);
    //speechResultArray=[speechResult];
    var regArray=[];
    joinReg='[s]?[\\W]{0,4}';
    if($(".mode").val()=="mots")
    {
      ...
      for(k in speechResultArray){
        regArray[k]=new RegExp("\\W([-]?[ ]?"+lettre_lt(speechResultArray[k])+joinReg+")\\W");
      }
      console.log("mode Mot",regArray);
    }
    else
    {regArray[0]=new RegExp("\\W([-]?[ ]?"+lettre_lt(speechResultArray.join(joinReg))+joinReg+")\\W");
      console.log("mode Phrase",regArray);
    }

    for(k in regArray)
    {
      reg=regArray[k];
      //var reg=new RegExp("\\b"+lettre_lt(speechResultArray[s])+"[s]?\\b", 'g');
      //reg=/\b/"+speechResultArray[s]/"+\b/g;
      sentFound=false;
      var SmallestNode="";
      var SmallestLengthNode=10000;
      $('.dialog').find("*").each(function(){
        var text=$(this).text();
        var html=$(this).html();
        //if(text==html)
        //{
          if(lettre_lt(text.toLowerCase()).replace(reg,"xxxxxxx")!=lettre_lt(text.toLowerCase())){
            if(text.length<SmallestLengthNode){
              SmallestLengthNode=$(this).text().length;
              SmallestNode=$(this);
            }
          }
        //}
      ...
      if(SmallestNode!="")
      {var text=SmallestNode.text();
        result=lettre_lt(text.toLowerCase()).match(reg);
        regStartIndex=result.index;
        regLength=result[0].length;
        newText=text.slice(0, regStartIndex)+"</span><span class='dialog_item'>"+text.slice(regStartIndex, regStartIndex+regLength)+"</span><span>"+text.slice(regStartIndex+regLength);
      SmallestNode.html("<span>"+newText+"</span>");
      }
      //dialog=$(".dialog").html().replace(reg,"<span class='dialog_item'>"+speechResultArray[s]+"</span>");
      //$("#reponse").html("<div class='dialog'>"+dialog+"</div>");
    //if($(this).hasClass("dialog_item")){foundLen+=parseInt($(this).text().length);};
    DialogObj=$(".dialog").clone();
    DialogObj.find(".dialog_item").remove();
    foundLen=parseInt(DialogObj.text().length);
    DialogObj.remove();
    dialogLen=parseInt($(".dialog").text().length);
    console.log(foundLen,dialogLen);
    pcentFound=Math.round((dialogLen-foundLen)*100/dialogLen);
    if(pcentFound>95){$('.dialog').find("*").css("color","black");pcentFound=100;}
...
{
len=selected_card_done.length+1;
nbre_mot_total=selected_card.length;
if(nbre_mot_total==1){$("#mots_restants").html(len+"/"+nbre_mot_total+" carte");}
else{$("#mots_restants").html(len+"/"+nbre_mot_total+" cartes");}
}
