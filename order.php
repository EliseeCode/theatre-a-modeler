...
function update_mots_restant_lettre()
{
len=selected_card_done.length;
nbre_mot_total=selected_card.length;

  if(len==1){$("#mots_restants").html(len+"/"+nbre_mot_total+" carte");}
...
function afficher_lettre(card_id)
{
  ...
  $(".lettre").on("click", function() {
    ...
    if(mot[id_lettre].toLowerCase()==mot[rg_lettre_a_trouver].toLowerCase())//SI LA LETTRE TAPE EST BONNE
      {
          bonne_lettre(id_lettre,"lettre",rg_lettre_a_trouver);
          rg_lettre_a_trouver++;
      }
...
if(lettre_lt(lettre_tape).toLowerCase()==lettre_lt(mot[rg_lettre_a_trouver]).toLowerCase())//SI LA LETTRE TAPE EST BONNE
    {
      ...
      bonne_lettre(id_lettre,"clavier",rg_lettre_a_trouver);
      rg_lettre_a_trouver++;
    }
else//SI LA LETTRE EST MAUVAISE
    {
...
function bonne_lettre(id_lettre,type,rg_lettre)
{
...
function fin_lettre()
{
  ...
  $('.consigne').html('Souhaites-tu valider ces mots ?<br><div class="boutton boutton_validation">Oui</div><div class="boutton boutton_retour">Non pas encore</div>');
}
</script>
