...
switch(game) {
  ...
  <br><- et -> pour passer
  <br>barre d'espace pour afficher la réponse, Entrée pour ne plus voir la carte.`);
  ...
  <span class="navBefore" onclick="affichage('discover',`+card_idBefore+`);"><img src="img/arrow_left.png" style="width:50px;margin:30px;"></span><span onclick='$( ".mot_card" ).show();'>show</span><span class="navAfter"  onclick="affichage('discover',`+card_idAfter+`);"><img src="img/arrow_left.png" style="width:50px;margin:30px;transform: scaleX(-1);"></span></div>`);
   $("body").off();
  
    $("body").on("keydown",function(e){
    console.log(e.which);
    if(e.which==37){
      affichage('discover',card_idBefore);
    }
    else if(e.which==39)
    {
      affichage('discover',card_idAfter);
    }
    else if(e.which==40 || e.which==32)
    {
      $( ".mot_card" ).show();
    }
    else if(e.which==13){
